...
export const Register: React.FC = (props) => {
    ...
    const testCredential =(username:string,password:string,email:string)=>{
        const lenghtError='invalid username or password'
        const takenCredential='username or Email already taken'
        const test =  () => {    
                api.get(apiUrl).then(res=>{
                res.data.map((value:any)=>{
                    if(user.username === value.username ||user.email === value.email){
                        window.alert(takenCredential)
                    }
                    else if(user.username.length<5 || user.password.length<8 ){
                        window.alert(lenghtError)
                    }
                    else{
